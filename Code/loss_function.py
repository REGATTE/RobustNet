"""
This module provides a set of tools for calculating loss functions in deep learning models, 
especially tailored for tasks like image segmentation and classification. It includes support for 
class-weighted and image-weighted losses, alongside standard and custom cross-entropy losses. 
These tools are designed to work seamlessly with PyTorch and support GPU acceleration.

To utilize these loss functions, instantiate the desired loss class and pass the model's output 
and target tensors to the forward method of the loss instance. Make sure to configure the 
loss function appropriately using the provided initialization parameters.
"""
...
class LossUtils:
    """
    Utility class for selecting and configuring loss functions based on provided command-line arguments.
    """
    def __init__(self) -> None:
        super(LossUtils, self).__init__()
        pass
    ...
    def get_loss(args):
        ...
        """
        if args.cls_wt_loss:
            ce_weight = torch.Tensor([0.8373, 0.9180, 0.8660, 1.0345, 1.0166, 0.9969, 0.9754, 1.0489, 0.8786, 1.0023, 0.9539, 0.9843, 1.1116, 0.9037, 1.0865, 1.0955, 1.0865, 1.1529, 1.0507])
        else:
            ce_weight = None

        if args.img_wt_loss:
            criterion = ImageBasedCrossEntropyLoss2d(classes=datasets.num_classes, size_average=True, ignore_index=datasets.ignore_label, upper_bound=args.wt_bound).cuda()
        elif args.jointwtborder:
            criterion = ImgWtLossSoftNLL(classes=datasets.num_classes, ignore_index=datasets.ignore_label, upper_bound=args.wt_bound).cuda()
        else:
    ...
    def get_loss_by_epoch(args):
        ...
        if args.img_wt_loss:
            ...
            criterion = ImgWtLossSoftNLL_by_epoch(classes=datasets.num_classes, ignore_index=datasets.ignore_label, upper_bound=args.wt_bound).cuda()
        else:
    ...
    def get_loss_bcelogit(args):
        """
        Configures a binary cross-entropy loss with logits, useful for binary classification tasks.
        args: commandline arguments
        return: criterion
        """
        if args.cls_wt_loss:
            pos_weight = torch.Tensor([0.8373, 0.9180, 0.8660, 1.0345, 1.0166, 0.9969, 0.9754, 1.0489, 0.8786, 1.0023, 0.9539, 0.9843, 1.1116, 0.9037, 1.0865, 1.0955, 1.0865, 1.1529, 1.0507])
        else:
            pos_weight = None
    ...
    def weighted_binary_cross_entropy(output, target):
        """
        Computes a weighted binary cross-entropy loss, allowing for differential weighting of classes.
        Args:
...
class L1Loss(nn.Module):
    """
    A simple L1 loss module for calculating the mean absolute error between two inputs.
    """
    def __init__(self):
...
def customsoftmax(inp, multihotmask):
    ...
    Custom Softmax
    """
    soft = torch_functional.softmax(inp, dim=1)
...
class ImgWtLossSoftNLL(nn.Module):
    """
    A variant of the image-weighted loss that applies a soft normalization and loss calculation, potentially useful for tasks with highly imbalanced classes.
    """
    ...
    def calculate_weights(self, target):
        """
        Calculate weights of the classes based on training crop or target distribution
        """
    ...
    def custom_nll(self, inputs, target, class_weights, border_weights, mask):
        ...
        Custom loss calculation that incorporates class weights and border weights.
        """
...
class ImgWtLossSoftNLL_by_epoch(nn.Module):
    """
    Similar to ImgWtLossSoftNLL, but designed to adjust the loss calculation based on the training epoch, allowing for dynamic adaptation over the course of training.
    """
